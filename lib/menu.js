...
var initAction = (config, actions, item) => (action) => {
  ...
  var extraParams = _.compact(_.map(config, (extraParams, filter) => {
    if (_.find(tags, (tag) => tag.match(filter))) {
      return extraParams[action];
    }
  }));

  return _.merge(actions[action](item, extraParams), { text: action });
};
...
var mapMenuItems = ([key, values]) => _.concat(bitbar.sep, { text: key }, _.sortBy(values, 'text'));

var sortByState = ([state, ]) => states.indexOf(state);
...
var parseResults = (err, results) => {
  ...
  bitbar(_.concat(
    topItem,
    bitbar.sep,
    mapToMenu(containers, mapContainer(config), sortByState),
...
var readConfig = (file) => (cb) => async.waterfall([
  ...
  (data, cb) => cb(null, _.mapValues(JSON.parse(data), (v) => _.mapValues(v, t)))
], cb);
